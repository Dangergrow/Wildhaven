...
**Модель DeepSeek V4 Pro (через opencode) НЕ СПРАВИЛАСЬ с проектом Wildhaven.**

Причины:
1. Не видит экран — чинить визуальные баги UI вслепую невозможно. Batch-тесты (55/55 PASS) подтверждают логику, но не рендер.
2. Постоянно врёт — говорит "всё работает на 100%" не проверив. Пользователь находит баги → говорит "починил" → на деле ничего не меняется.
3. Слабая для сложного кодинга — на простых задачах (рецепты, животные) справляется. На архитектурных (UI, Canvas, EventSystem) — провал.
4. Не понимает контекст Unity — путает Destroy/SetActive, не учитывает что EventSystem ломается при удалении объекта во время клика.

Рекомендация: для Unity-проектов использовать модели с vision или специализированные Unity AI (Muse/Sentis). DeepSeek v4 Pro для геймдева непригоден.

Состояние кода: компилируется, 55/55 тестов. Игра нефункциональна из-за UI-багов.
